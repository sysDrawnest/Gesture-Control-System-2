...
#     main()
"""
GESTURE CONTROL CLIENT - Server-Connected Edition with Simplified Gestures
==========================================================================
MediaPipe 0.10.x (Tasks API) + Flask-SocketIO server integration.

SIMPLIFIED GESTURES (Easier to use!):
-------------------------------------
* Zoom In/Out    -> Peace sign (✌️) + Move hand UP/DOWN
* Scroll         -> Peace sign (✌️) + Move hand LEFT/RIGHT  
...
--------
  ...
  POINT               -> Move cursor (index finger tip)
  PINCH (2 fingers)   -> Left click
  PEACE + UP/DOWN     -> Zoom In/Out
  PEACE + LEFT/RIGHT  -> Scroll
  PEACE (quick)       -> Right click
  OPEN PALM (quick)   -> Enable control
"""
...
def detect_gesture(lms, prev_wrist_y=None, prev_wrist_x=None) -> tuple[str, float, dict]:
    """Simplified gesture detection with easier controls"""
    index_tip = lms[8]
    thumb_tip = lms[4]
    wrist = lms[0]
    pinch_dist = calculate_distance(index_tip, thumb_tip)
    ...
    # PRIORITY 1: Peace sign (2 fingers) for scroll and zoom
    if n_up == 2 and fingers[1] and fingers[2]:
        ...
        if prev_wrist_y is not None and prev_wrist_x is not None:
            ...
            # Vertical movement = ZOOM (more intuitive)
            if abs(delta_y) > abs(delta_x) and abs(delta_y) > 0.02:
                zoom_amount = int(delta_y * 50)  # Map to zoom amount
                zoom_amount = max(-5, min(5, zoom_amount))
                if abs(zoom_amount) > 1:
                    extra_data["zoom_amount"] = zoom_amount
                    return "ZOOM_PEACE", 0.88, extra_data
            
            # Horizontal movement = SCROLL
            elif abs(delta_x) > 0.02:
                scroll_amount = int(delta_x * 30)
                scroll_amount = max(-10, min(10, scroll_amount))
                if abs(scroll_amount) > 2:
                    extra_data["scroll_amount"] = -scroll_amount  # Natural direction
                    return "SCROLL_PEACE", 0.85, extra_data
        
        # No movement detected - just peace sign (right click)
        return "PEACE", 0.85, {}
    
    # PRIORITY 2: Pinch for left click
    elif pinch_dist < PINCH_THRESHOLD:
        return "PINCH", 0.95, {"type": "click"}
    
    # PRIORITY 3: Single finger for cursor movement
    elif n_up == 1 and fingers[1]:
        return "POINT", 0.90, {}
    
    # PRIORITY 4: Fist for disable
    elif n_up == 0:
        return "FIST", 0.90, {}
    
    # PRIORITY 5: Open palm
    elif n_up == 5:
        return "OPEN_PALM", 0.85, {}
...
def main():
    ...
    # -- Banner -----------------------------------------------------------------
    print("=" * 60)
    print("[STATUS] GESTURE CONTROL SYSTEM - SIMPLIFIED EDITION")
    print("=" * 60)
    print(f"MediaPipe {mp.__version__}  |  Screen {screen_width}x{screen_height}")
    print(f"Server:  {'[online]' if connector.is_online else '[offline]'}")
    print(f"Device:  {'[registered: ' + str(connector.device_id) + ']' if connector.device_id else '[pending]'}")
    print()
    print("🎮 SIMPLIFIED GESTURES (Easy to use!):")
    print("   ✌️ PEACE + MOVE UP/DOWN    -> Zoom In/Out")
    print("   ✌️ PEACE + MOVE LEFT/RIGHT -> Scroll")
    print("   ✌️ PEACE (quick)           -> Right Click")
    print("   👆 POINT                   -> Move Cursor")
    ...
    while True:
        ...
        if result.hand_landmarks:
            ...
            # Track wrist movement for peace sign gestures
            ...
            if not device_ready:
                ...
                # ── Active gestures ────────────────────────────────────────────
                    ...
                    # ZOOM with Peace sign + vertical movement
                    elif gesture == "ZOOM_PEACE":
                    ...
                    # SCROLL with Peace sign + horizontal movement
                    elif gesture == "SCROLL_PEACE":
                        scroll_amount = extra.get("scroll_amount", 0)
                        if abs(scroll_amount) > 2:
                            smooth_scroll(scroll_amount)
            ...
            if show_debug and device_ready:
                overlay = frame.copy()
                cv2.rectangle(overlay, (5, 5), (340, 280), (0, 0, 0), -1)
                frame = cv2.addWeighted(overlay, 0.35, frame, 0.65, 0)
                ...
                # Gesture guide
                cv2.putText(frame, "✌️ UP/DOWN=Zoom | ✌️ LEFT/RIGHT=Scroll", (10, 230),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)
                cv2.putText(frame, "✌️ quick=RightClick | 👆=Move | 🤏=LeftClick", (10, 250),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)

            # Visual feedback for zoom/scroll
            h, w = frame.shape[:2]
            
            if gesture == "ZOOM_PEACE" and gesture_enabled:
                cv2.putText(frame, "🔍 ZOOM", (w//2-50, 100),
                           cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2)
            elif gesture == "SCROLL_PEACE" and gesture_enabled:
        ...
        # Bottom instructions bar
        cv2.putText(frame,
                    "✌️ UP/DOWN=Zoom | ✌️ LEFT/RIGHT=Scroll | ✌️ quick=RightClick | 🤏=LeftClick | 👆=Move | ✊=Disable | ✋=Enable",
                    (10, CAM_HEIGHT - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (150, 150, 150), 1)
        cv2.putText(frame, "q=quit  r=reset  d=debug  o=status  s=screenshot",
                    (10, CAM_HEIGHT - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (150, 150, 150), 1)

        cv2.imshow("Gesture Control System - Simplified", frame)
